...
# The purpose of this file is implementation of prediction for AlBERT Model
import os, torch, timeit,sys
...
def do_prediction(model_dir, model_name, questions_dir):
    ...
    output_prediction_file = os.path.join(model_dir, model_name + "_albert_predictions_sfu.json")
    output_nbest_file = os.path.join(model_dir, model_name + "_albert_nbest_predictions_sfu.json")
    output_null_log_odds_file = os.path.join(model_dir, model_name + "_null_odds_sfu.json")
...
def main(MODEL_DIR,questions):
    # MODEL_DIR = './models/albert'
    for name in os.listdir(MODEL_DIR):
        if os.path.isdir(os.path.join(MODEL_DIR, name)):
            print('Do predictions on ' + questions + ' for albert: ' + name)
            do_prediction(os.path.join(MODEL_DIR, name),name,questions)
